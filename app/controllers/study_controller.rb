class StudyController < ApplicationController
  ...
  def index
    @topics = TOPICS
    @study_modules = StudyModule.published.visible_to(current_user).order(:position)
    @study_progresses = current_user ? current_user.study_progresses.index_by(&:study_slug) : {}
  end

  def show
    @topic = TOPICS[params[:slug]]
    @study_module = StudyModule.published.visible_to(current_user).find_by(slug: params[:slug]) unless @topic
